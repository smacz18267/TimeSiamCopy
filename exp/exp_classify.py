...
class Exp_Classify:
    ...
    # ======================= Collate: pad/truncate to (fixed_C, fixed_L) =======================
    ...
    def _pad_to_fixed(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.ndim == 3:
            ...
            x = x.reshape(C * F, L)
        C, L = x.shape
        C_new, L_new = min(self.fixed_C, C), min(self.fixed_L, L)
    ...
    def train(self, setting=None):
        ...
        ) if (self.cfg.pair_mode and self.cfg.contrastive_weight > 0) else None
        ...
        ) if (self.cfg.ce_weight > 0) else None
        ...
        for epoch in range(self.cfg.epochs):
            ...
            it_pair = iter(train_pair_loader) if train_pair_loader is not None else None
            it_ce   = iter(train_ce_loader) if train_ce_loader is not None else None
            ...
            for _ in range(steps):
                ...
                if it_pair is not None:
                    try:
                        loss_c = self._step_pair(next(it_pair)) * self.cfg.contrastive_weight
                    except StopIteration:
                        it_pair = iter(train_pair_loader)

                if it_ce is not None:
                    try:
                        loss_e = self._step_ce(next(it_ce)) * self.cfg.ce_weight
                    except StopIteration:
